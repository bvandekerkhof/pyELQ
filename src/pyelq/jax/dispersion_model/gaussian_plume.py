...
class GaussianPlume:
    ...
    def compute_coupling_array(
        ...
        Args:
            ...
            theta (jnp.ndarray): Mathematical wind direction at source locations [radians]:
                calculated as jnp.arctan2(v_component_wind, u_component_wind).
    ...
    def compute_coupling_ground(
        ...
        plume_coupling = self.compute_coupling_array(
            jnp.array(relative_x),
            jnp.array(relative_y),
            jnp.array(z_sensor),
            jnp.array(z_source),
            jnp.array(wind_speed[:, :, None]),
            jnp.array(theta[:, :, None]),
            jnp.array(wind_turbulence_horizontal[:, :, None]),
            jnp.array(wind_turbulence_vertical[:, :, None]),
            jnp.array(gas_density[:, :, None]),
        )
...
# @partial(jit, static_argnames = ['source_half_width', 'minimum_contribution'])
@jax.jit
def _compute_coupling_array_jax(
    sensor_x: jnp.ndarray,
    sensor_y: jnp.ndarray,
    sensor_z: jnp.ndarray,
    source_z: jnp.ndarray,
    ...
    Args:
        ...
        wind_turbulence_vertical (jnp.ndarray): Vertical wind turbulence [deg].
        gas_density (Union[float, jnp.ndarray]): Gas density to use in coupling calculation [kg/m^3].
        source_half_width (float): Half width of the source [m].
        minimum_contribution (float): Minimum contribution to be considered in the Gaussian plume coupling.
